from dafne.data.datasets.ucas_aod import load_annotation, parse_annotation
...
class UcasAodEvaluator(DafneEvaluator):
...
"""
    ...
    detpath is the path for 15 result files, for the format, you can refer to "http://captain.whu.edu.cn/UCASAODweb/tasks.html"
...
label2name = dict((label, name) for label, name in enumerate(classnames))
name2label = dict((name, label) for label, name in enumerate(classnames))
